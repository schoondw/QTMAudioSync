function [y,Fs,bits,opt_ck] = bwfread(varargin)
%BWFREAD Extension of WAVREAD function for Broadcast Wave Format (BWF)
%   as specified by the European Broadcasting Union (version 1)
% 
%   Use of bwfread exactly the same as wavread.
%
%   Extensions:
%   - OPTS contains an additional '.bext' field with meta information
%     if the .wav file is a BWF
%   - Retrieve meta-information without loading the data:
%     OPTS = bwfread(FILE,'info')
%     (In this case an additional fields .nChannels and .nSamples are added
%      to OPTS for complete size information)
%   - A file dialog is opened when no input arguments are specified or if
%     the first argument is empty
%
%
%   Adapted by: Erwin Schoonderwaldt, March 2011
%
%----------------------
%   Original help text:
%
...
%   $Revision: 1.1.6.10 $  $Date: 2008/04/21 16:25:02 $
...
while ~end_of_file
   ...
   % --- ES: additional case for reading meta-data (<bext> chunk in .bwf
   %         format)
   case 'bext'
       % <bext-ck> found (bwf format)
      [opt_ck,msg] = read_wavebext(fid,ck,opt_ck);
      if ~isempty(msg)
          error('wavread:InvalidFile',msg);
      end
      % if rem(ck.Size,2), ck.Size=ck.Size+1; end
%    % --- ES: end additional case
   ...
   case 'data'
      ...
      elseif strcmpi(ext,'info')
         % Caller doesn't want data - just meta information:
         [samples,msg] = read_wavedat(ck, opt_ck.fmt, -1, isNative);
         if ~isempty(msg)
             error('wavread:InvalidFile',msg);
         end
         y = opt_ck;
         y.nChannels = opt_ck.fmt.nChannels;
         y.nSamples = samples;
...
% --- ES: additional subfunction
% ---------------------------------------------
% READ_WAVEBEXT: Read BEXT chunk (adapted from read_wavefmt).
%   Assumes fid points to the <bext> subchunk.
%   Requires chunk structure to be passed, indicating
%   the length of the chunk in case we don't recognize
%   the format tag.
% ---------------------------------------------
function [opt_ck,msg] = read_wavebext(fid,ck,orig_opt_ck)

opt_ck = orig_opt_ck;

orig_pos    = ftell(fid);
total_bytes = ck.Size; % # bytes in subchunk
nbytes      = 602;  % # of required bytes in <wave-format> header
msg = '';
err_msg = 'Error reading <bext> chunk.';
...
opt_ck.bext.Description = deblank(fread(fid,256,'*char')');
opt_ck.bext.Originator = deblank(fread(fid,32,'*char')');
opt_ck.bext.OriginatorReference = deblank(fread(fid,32,'*char')');
opt_ck.bext.OriginationDate = fread(fid,10,'*char')';
opt_ck.bext.OriginationTime = fread(fid,8,'*char')';
opt_ck.bext.TimeReference = fread(fid,1,'uint64');
opt_ck.bext.Version = fread(fid,1,'uint16');
if opt_ck.bext.Version > 0 %if opt_ck.bext.Version == 1
    opt_ck.bext.UMID = dec2hex(fread(fid,64));
    fseek(fid,190,'cof');
else
    fseek(fid,254,'cof');
end

% Optional: CodingHistory
if rem(total_bytes,2), total_bytes=total_bytes+1; end
rbytes = total_bytes - (ftell(fid) - orig_pos);
if rbytes>0 % if rbytes~=0
    % opt_ck.bext.CodingHistory = fread(fid,rbytes,'*char')';
    opt_ck.bext.CodingHistory = deblank(fread(fid,rbytes,'*char')');
end
% --- ES: end of addition
...
% --- ES: added file dialog
if nargin==0 || isempty(varargin{1})
    [fstr,pstr]=uigetfile('*.wav','Open WAV file');
    if fstr
        file=fullfile(pstr,fstr);
        if nargin > 0, varargin(1)=[]; end
    else
        error('wavread:TooFewArguments', ...
            'File not specified.');
    end
else % standard wavread
    file = varargin{1};
    varargin(1)=[];
end
...
fmt=varargin{end};
if ischar(fmt)
    idx = find(strncmpi(fmt,{'double','native','size','info'},numel(fmt)));
    if isempty(idx)
...
if isInfo && (nargs>0)
    error('wavread:SizeAndSampleRange', ...
        'Too many arguments with ''info'' option.');
end
